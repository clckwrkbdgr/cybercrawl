...
std::string input_name()
{
	...
	while(true) {
		...
		if (your_nam == "bones" || your_nam.size() == 0) {
			cprintf(EOL"That's a silly name!"EOL);
			continue;
		}
		for (size_t glorpo = 0; glorpo < your_nam.size(); glorpo ++) {
			if (your_nam [glorpo] == ' ') {
...
void apply_job_fighter(int & weap_skill)
{
...
void apply_job_gladiator(int & weap_skill)
{
...
void apply_job_crusader(int & weap_skill)
{
...
void apply_job_death_knight(int & weap_skill)
{
...
void apply_job_chaos_knight(int & weap_skill)
{
...
void apply_job_reaver(int & weap_skill)
{
...
int apply_stats_from_species(int & keyn)
{
	switch(keyn) {
...
switch_start :
	int result = apply_stats_from_species(keyn);
	if(result == 1) {
		goto switch_start;
	} else if(result == 2) {
		goto spec_query;
...
query5:
	if (keyn == 'a') you[0].clas = JOB_FIGHTER;
...
int new_game(void)
{
	...
	you[0].your_name = input_name();

	if(char_already_exists()) {
...
spec_query2 :
	...
	switch(you[0].clas)
	{
		case JOB_FIGHTER: apply_job_fighter(weap_skill); break;
		case JOB_WIZARD: apply_job_wizard(); break;
		case JOB_PRIEST: apply_job_priest(); break;
		case JOB_THIEF: apply_job_thief(); break;
		case JOB_GLADIATOR: apply_job_gladiator(weap_skill); break;
		case JOB_NECROMANCER: apply_job_necromancer(); break;
		case JOB_PALADIN: apply_job_paladin(); break;
		case JOB_ASSASSIN: apply_job_assassin(); break;
		case JOB_BERSERKER: apply_job_berserker(); break;
		case JOB_RANGER: apply_job_ranger(); break;
		case JOB_CONJURER:
		case JOB_ENCHANTER:
		case JOB_SUMMONER:
		...
		case JOB_WARPER:
					apply_job_mage();
					break;
		case JOB_CRUSADER: apply_job_crusader(weap_skill); break;
		case JOB_DEATH_KNIGHT: apply_job_death_knight(weap_skill); break;
		case JOB_CHAOS_KNIGHT: apply_job_chaos_knight(weap_skill); break;
		case JOB_HEALER: apply_job_healer(); break;
		case JOB_REAVER: apply_job_reaver(weap_skill); break;
		case JOB_STALKER: apply_job_stalker(); break;
		case JOB_MONK: apply_job_monk(); break;
	}
	...
	for (fi2 = 0; fi2 < 30; fi2 ++)
	{
		for (fi = 0; fi < 50; fi ++)
		{
			...
			std::string name = (you[0].your_name.size() > 6) ? std::string(you[0].your_name, 6) : you[0].your_name;
			format << name << ((fi < 10) ? "0" : "") << fi << char(fi2 + 97);
